...
# fake navbar using a radio (fast + simple)
nav_pages = [
    "Overview",
    "Ministries",
...
# ================== PAGE: MINISTRIES ==================
    ...
    if not ministry_col:
        ...
        if not ministries:
            ...
            # BE/RE/AE comparison if present
            if be_col and re_col and ae_col:
                ...
                df_be = latest_min[[ministry_col, be_col, re_col, ae_col, year_col]].groupby(ministry_col)[cols].sum()
...
# ================== PAGE: STATES ==================
    ...
    if not state_col:
        ...
        # simple state-level chart + table (map needs geojson, which you can add later)
        agg_state = (
        ...
        st.caption("You can later plug this into a choropleth map using Plotly and an India states GeoJSON file.")

# ================== PAGE: FORECASTING ==================
elif page == "Forecasting":
    st.markdown("<div class='section-title'>Budget forecasting</div>", unsafe_allow_html=True)
    st.caption("Simple time-series forecast using Exponential Smoothing (Holt-Winters).")

    # use total measure by year
    agg = df.groupby(year_col)[measure].sum().reset_index()
    agg = agg.sort_values(year_col)

    if len(agg) < 4:
        st.warning("Not enough years of data for a reliable forecast (need at least 4).")
    else:
        # fit model
        series = agg.set_index(year_col)[measure]
        model = ExponentialSmoothing(series, trend="add", seasonal=None)
        try:
            fit = model.fit(optimized=True)
            forecast_years = 5
            future_index = range(int(agg[year_col].max()) + 1, int(agg[year_col].max()) + 1 + forecast_years)
            forecast_vals = fit.forecast(forecast_years)

            df_forecast = pd.DataFrame({
                year_col: list(series.index) + list(future_index),
                "Historical": list(series.values) + [np.nan] * forecast_years,
                "Forecast": [np.nan] * len(series) + list(forecast_vals.values),
            }).set_index(year_col)

            st.line_chart(df_forecast, height=360)

            st.markdown("**Forecast table (next 5 years)**")
            st.dataframe(
                pd.DataFrame({
                    year_col: list(future_index),
            ...
            last_hist = series.iloc[-1]
            last_forecast = forecast_vals.iloc[-1]
            if last_hist != 0:
                change = (last_forecast - last_hist) / last_hist * 100
                direction = "higher" if change >= 0 else "lower"
                st.markdown(
                    f"> By {future_index[-1]}, total **{measure}** is projected to be **{fmt_amt(last_forecast)}**, "
                    f"about **{change:+.1f}% {direction}** than {series.index[-1]}."
                )
        except Exception as e:
            st.error("Forecasting model failed to fit.")
            st.exception(e)

# ================== PAGE: AI INSIGHTS (rule-based mini assistant) ==================
elif page == "AI Insights":
    st.markdown("<div class='section-title'>AI-style Insights Assistant (offline)</div>", unsafe_allow_html=True)
    st.caption("This is a lightweight rule-based assistant (no external API). Type a question about the data.")
    ...
    if q.strip():
        ...
        if "which" in q_low and ("highest" in q_low or "largest" in q_low or "most" in q_low) and ("increase" in q_low or "growth" in q_low):
            ...
            if not target_dim:
                ...
                if df_window.empty:
                    ...
                    # compute growth from first to last year inside that window
        ...
        # Example 2: total for a ministry/state
        if any(w in q_low for w in ["total", "allocation", "spending", "expenditure"]) and ("for" in q_low or "of" in q_low):
            ...
            if target_dim:
                import re
                # crude attempt: look for a word starting with capital from user side – but we don't know; fallback to top one
...
# ================== PAGE: ABOUT ==================
    ...
    st.markdown(
        ...
        **Technical stack**
        ...
        - `statsmodels` (Holt-Winters) for simple forecasting  
